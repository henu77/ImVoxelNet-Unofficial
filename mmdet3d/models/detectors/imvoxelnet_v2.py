...
class ImVoxelNet_v2(BaseDetector):
    def __init__(self,
                 backbone,
                 neck,
                 neck_3d,
    ...
    #     x = self.backbone(img)
    #     features_2d = self.head_2d.forward(x[-1], batch_img_metas) if self.head_2d is not None else None
    #     x = self.neck(x)[0]
    ...
    #         angles = features_2d[0] if features_2d is not None and self.mode == 'test' else None
    ...
    def loss(self, batch_inputs_dict: dict, batch_data_samples: SampleList,
        ...
        x, valids, features_2d = self.extract_feat(batch_inputs_dict,
                                                   batch_data_samples)
        losses = self.bbox_head.forward_train(x, valids.float(),
    ...
    def predict(self, batch_inputs_dict: dict, batch_data_samples: SampleList,
        ...
        x, valids, features_2d = self.extract_feat(batch_inputs_dict,
                                                   batch_data_samples)
        x = self.bbox_head(x)
        bbox_list = self.bbox_head.get_bboxes(*x, valids.float(),
                                              batch_data_samples, **kwargs)
        bbox_results = [
        ...
        if self.head_2d is not None:
            angles, layouts = self.head_2d.get_bboxes(*features_2d,
                                                      batch_data_samples)
            for i in range(len(batch_inputs_dict['imgs'])):
                bbox_results[i]['angles'] = angles[i]
                bbox_results[i]['layout'] = layouts[i]
        return bbox_results
    def _forward(self, batch_inputs, batch_data_samples = None):
        x, valids, features_2d = self.extract_feat(batch_inputs, batch_data_samples)
        losses = self.bbox_head.forward_train(x, valids.float(), batch_data_samples)
        return losses
    ...
    def _compute_projection(img_meta, stride, angles):
        projection = []
        intrinsic = torch.tensor(img_meta['lidar2img']['intrinsic'][:3, :3], dtype=torch.float32)
